...
export function HomeHero() {
  ...
  return (
    <section className="relative isolate overflow-hidden border-b border-gray-300 bg-white">
      ...
      <div className="absolute inset-0 hidden lg:block">
        <div className="absolute left-16 top-24 h-14 w-14 rounded-full border border-yellow-400 bg-yellow-200 blur-[0.5px] animate-floatUp opacity-40" />
        <div className="absolute left-1/3 top-10 h-9 w-9 rounded-full border border-yellow-300 bg-yellow-100 animate-floatUp [animation-delay:1s] opacity-50" />
        <div className="absolute right-24 top-36 h-12 w-12 rounded-full border border-yellow-400 bg-yellow-200 animate-floatUp [animation-delay:2s] opacity-40" />
      </div>
      ...
      <div className="relative mx-auto grid min-h-[92vh] w-full max-w-7xl items-center gap-14 px-4 py-20 sm:px-6 lg:grid-cols-[1.15fr_0.85fr] lg:px-8 lg:py-24">
        <div className="max-w-4xl">
          ...
          <h1 className="mt-6 font-display text-5xl font-black uppercase leading-[0.95] tracking-[-0.04em] text-[#1a1a1a] sm:text-6xl lg:text-8xl">
          ...
          <div className="mt-10 grid gap-4 sm:grid-cols-3">
            {stats.slice(0, 3).map((stat) => (
              <GlassCard key={stat.label} className="p-5">
                <p className="text-xs uppercase tracking-[0.28em] text-gray-600">{stat.label}</p>
                <div className="mt-3 flex items-end justify-between gap-4">
                  <StatNumber value={stat.value} />
                  <span className="max-w-[9rem] text-xs leading-5 text-gray-600">{stat.detail}</span>
                </div>
              </GlassCard>
            ))}
          </div>
        ...
        <GlassCard className="relative overflow-hidden p-4 lg:p-5">
          <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_right,rgba(255,213,0,0.15),transparent_30%),linear-gradient(135deg,rgba(255,213,0,0.08),rgba(255,213,0,0.02))]" />
          <div className="relative overflow-hidden rounded-[1.25rem] border border-yellow-300 bg-yellow-100">
            ...
            <div className="absolute inset-x-4 bottom-4 rounded-[1.2rem] border border-yellow-300 bg-white/90 p-4 backdrop-blur-xl shadow-md">
              <div className="flex items-start justify-between gap-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.28em] text-red-600">Featured talent</p>
                  <h2 className="mt-2 font-display text-3xl font-black text-[#1a1a1a]">{talents[6].name}</h2>
                  <p className="mt-1 text-sm text-gray-600">{talents[6].role} · {talents[6].city}</p>
                </div>
                <div className="rounded-2xl border border-red-300 bg-red-100 px-3 py-2 text-right shadow-sm">
                  <p className="font-accent text-3xl text-red-600">{talents[6].rating}</p>
                  <p className="text-[10px] uppercase tracking-[0.26em] text-red-600">Rating</p>
                </div>
              </div>
              <div className="mt-4 flex flex-wrap gap-2 text-xs text-gray-700">
                {talents[6].highlights.map((item) => (
                  <span key={item} className="rounded-full border border-yellow-300 bg-yellow-100 px-3 py-1 text-gray-700">{item}</span>
                ))}
              </div>
            </div>
          </div>
...
export function StatsSection() {
  return (
    <PageSection className="bg-[linear-gradient(180deg,#FFFEF9,#FFF9E6_50%,#FFFEF9)]">
        ...
        {stats.map((stat) => (
          <GlassCard key={stat.label} className="p-6">
            ...
            <div className="mt-4 flex items-end justify-between gap-4">
              <StatNumber value={stat.value} />
              <div className="h-14 w-14 rounded-full border border-yellow-300 bg-[radial-gradient(circle,rgba(255,213,0,0.3),transparent_65%)] shadow-[0_0_35px_rgba(255,213,0,0.12)]" />
            </div>
            <p className="mt-3 text-sm leading-6 text-gray-600">{stat.detail}</p>
          </GlassCard>
        ))}
      </div>
    </PageSection>
              ...
              <GlassCard className="flex h-full min-h-44 flex-col justify-between p-5 transition duration-300 group-hover:-translate-y-2">
                <div>
                  <div className="flex items-center justify-between">
                    <p className="text-xs uppercase tracking-[0.28em] text-red-600">0{index + 1}</p>
                    <span className="rounded-full border border-yellow-300 bg-yellow-100 px-2 py-1 text-[10px] uppercase tracking-[0.24em] text-yellow-800">Curated</span>
                  </div>
                  <h3 className="mt-4 font-display text-2xl font-black text-[#1a1a1a]">{category.name}</h3>
                  <p className="mt-3 text-sm leading-6 text-gray-600">{category.description}</p>
                </div>
                <div className="mt-6 flex items-center justify-between text-sm text-gray-700 font-medium">
...
export function FeaturedTalentsSection() {
  return (
    <PageSection className="bg-[linear-gradient(180deg,#FFFEF9,#FFF9E6_60%,#FFFEF9)]">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeading
  ...
  return (
    <PageSection className="bg-white">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeading
          eyebrow="Gallery"
          title="Editorial imagery with a brushed, reflective finish."
...
export function TestimonialsSection() {
  return (
    <PageSection className="bg-[linear-gradient(180deg,#FFFEF9,#FFF9E6_55%,#FFFEF9)]">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <SectionHeading
...
export function PartnersSection() {
  return (
    <PageSection className="border-y border-gray-300 bg-[linear-gradient(135deg,#fef3c7,#fde68a_40%,#fcd34d_70%,#fbcf24)] text-[#1a1a1a]">
      <div className="mx-auto w-full max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="overflow-hidden rounded-[2rem] border border-yellow-300 bg-white/70 backdrop-blur-xl shadow-md">
          <div className="animate-[scroll_28s_linear_infinite] whitespace-nowrap py-6 text-sm font-semibold uppercase tracking-[0.32em] text-[#1a1a1a] [animation-play-state:running]">
        ...
        <div className="mt-12 grid gap-4 md:grid-cols-2 xl:grid-cols-4">
          {processSteps.map((step, index) => (
            <GlassCard key={step.title} className="relative p-6">
              <div className="flex items-center justify-between">
                <span className="font-accent text-4xl text-red-600">0{index + 1}</span>
                <CheckCircle2 className="h-5 w-5 text-red-600" />
              </div>
              <h3 className="mt-6 font-display text-2xl font-black text-[#1a1a1a]">{step.title}</h3>
              <p className="mt-3 text-sm leading-6 text-gray-600">{step.detail}</p>
              <div className="absolute bottom-0 left-6 right-6 h-px bg-gradient-to-r from-red-400 via-yellow-300 to-transparent opacity-70" />
            </GlassCard>
          ))}
        </div>
      </div>
        ...
        <GlassCard className="p-8">
          ...
          <div className="mt-8 grid gap-4">
            {articles.slice(0, 3).map((article) => (
              <Link key={article.slug} href={`/blog/${article.slug}`} className="group rounded-[1.4rem] border border-yellow-200 bg-yellow-50 p-4 transition hover:bg-yellow-100">
                <div className="flex flex-wrap items-center gap-2 text-xs uppercase tracking-[0.24em] text-red-600 font-medium">
                  <span>{article.category}</span>
                  <span>•</span>
                  <span>{article.readTime}</span>
                </div>
                <h3 className="mt-3 font-display text-2xl font-black text-[#1a1a1a]">{article.title}</h3>
